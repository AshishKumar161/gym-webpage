...
import { User, Settings, Lock, Upload, Sparkles, Scale, Info } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [fitnessGoal, setFitnessGoal] = useState(user?.fitnessGoal ?? "GENERAL_FITNESS");
  ...
  return (
    <div className="space-y-10">
      {/* Header */}
      <div>
        <h1 className="font-heading font-black text-3xl text-white mb-2">Account Settings</h1>
        <p className="text-sm text-white/50">Manage your profile details, password, and custom fitness objectives.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="glass-card p-6 border-white/5">
            ...
            <form onSubmit={handleProfileSubmit} className="space-y-6">
              ...
              <div className="pt-4 border-t border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="text-xxs font-bold text-white/50 uppercase block mb-1">Height (cm)</label>
                    <input
                      type="number"
                      value={heightCm}
                  ...
                  <div>
                    <label className="text-xxs font-bold text-white/50 uppercase block mb-1">Weight (kg)</label>
                    <input
                      type="number"
                      value={weightKg}
                  ...
                  <div>
                    <label className="text-xxs font-bold text-white/50 uppercase block mb-1">Fitness Goal</label>
                    <select
                      value={fitnessGoal}
                      onChange={(e) => setFitnessGoal(e.target.value)}
                      className="w-full bg-dark border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-gold"
                    >
                  ...
                  <div>
                    <label className="text-xxs font-bold text-white/50 uppercase block mb-1">Fitness Level</label>
                    <select
                      value={fitnessLevel}
        ...
        {/* Password update & extra tools (Right Column) */}
        <div className="space-y-8">
          ...
          <div className="glass-card p-6 border-white/5">
            ...
            <p className="text-xxs text-white/50 leading-relaxed">
              Your profile values are synced with the trainers and AI recommendations engine to suggest workouts and diet custom blueprints. Make sure your Weight and Height are up-to-date!
            </p>
          </div>
        </div>
      </div>
