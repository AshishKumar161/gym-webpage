...
export async function renderMemberNotifications() {
  let notifications = [];
  try {
  ...
  return `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;">
      <div>
        <h2 class="dash-header-title">Notifications</h2>
        <p class="dash-subtitle" style="margin-bottom:0;">View announcements, reminders, and alerts from your gym.</p>
      </div>
    ...
    <div style="display:flex; flex-direction:column; gap:1rem;">
      ${notifications.map(n => `
        <div class="glass-card" style="padding:1.5rem; display:flex; align-items:flex-start; gap:1rem; ${n.read ? 'opacity:0.7;' : 'border-left: 4px solid var(--accent-cyan);'}">
          ...
          <div style="flex:1;">
            <h4 style="font-size:1.1rem; margin-bottom:0.3rem;">${n.title || 'New Alert'}</h4>
